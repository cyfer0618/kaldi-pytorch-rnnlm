...
#ifndef KALDI_PYRNNLM_PYTORCH_RNNLM_H_
#define KALDI_PYRNNLM_PYTORCH_RNNLM_H_
...
struct KaldiPyRnnlmWrapperOpts {
  std::string unk_symbol;
  int32 num_threads;  // 0 means unlimited

  KaldiPyRnnlmWrapperOpts() : unk_symbol("<oos>"), num_threads(1) {}

  void Register(OptionsItf *opts) {
    opts->Register("unk-symbol", &unk_symbol, "Symbol for out-of-vocabulary "
                   "words in rnnlm.");
    opts->Register("num-threads", &num_threads, "Number of threads for PY computation; "
                   "0 means unlimited.");
  }
};
...
class KaldiPyRnnlmWrapper {
 public:
  ...
  KaldiPyRnnlmWrapper(const KaldiPyRnnlmWrapperOpts &opts,
  ...
  const torch::Tensor& GetInitialContext() const;
  // auto GetInitialContext() const;
  ...
  const torch::Tensor& GetInitialCell() const;
  // auto GetInitialCell() const;
 ...
 private:
  ...
  void ReadPyModel(const std::string &py_model_path, int32 num_threads);
  ...
  KaldiPyRnnlmWrapperOpts opts_;

  /*
    (Samrat): Possibly better to change auto to torch::Tensor
  */

  //auto initial_context_;
  //auto initial_cell_;
  torch::Tensor initial_context_;
  torch::Tensor initial_cell_;
  ...
  KALDI_DISALLOW_COPY_AND_ASSIGN(KaldiPyRnnlmWrapper);
};

class PyRnnlmDeterministicFst:
 ...
 public:
  ...
  PyRnnlmDeterministicFst(int32 max_ngram_order, KaldiPyRnnlmWrapper *rnnlm);
  ~PyRnnlmDeterministicFst();
  void Clear();
 ...
 private:
  ...
  KaldiPyRnnlmWrapper *rnnlm_;
  int32 max_ngram_order_;
  std::vector<torch::Tensor*> state_to_context_;
  std::vector<torch::Tensor*> state_to_cell_;
};

}  // namespace tf_rnnlm
}  // namespace kaldi

#endif  // KALDI_PYRNNLM_PYTORCH_RNNLM_H_
